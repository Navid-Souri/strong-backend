...
class WorkoutPlanSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        for exercise_detail in exercises_details_data:
            ...
            exercise_instance = Exercise.objects.get(id=exercise_id)  # ✅ FIXED
            WorkoutPlanExercise.objects.create(
                workout_plan=workout_plan, exercise=exercise_instance, **exercise_detail
            )
    ...
    def update(self, instance, validated_data):
        ...
        if exercises_details_data is not None:
            ...
            existing_wpe_ids = set(
                instance.workoutplanexercise_set.values_list("exercise_id", flat=True)
            )
    ...
    def validate(self, data):
        ...
        if not any(field in data for field in metrics):
            raise serializers.ValidationError(
                "حداقل یکی از فیلدهای تعداد تکرار، وزن، مسافت یا زمان باید پر شود."
            )
        return data
